...
// skipThoughtSignatureValidator is Google's documented bypass value for a
// functionCall part whose real thoughtSignature isn't available — used here
// on a geminiThoughtSigs cache miss (TTL expiry, a mid-conversation miroxy
// restart, or replaying history assembled before this cache existed) so the
// request degrades to skipped validation instead of a hard 400. See
