// Package wireformat implements bidirectional wire-format converters, one
// per LLM protocol dialect, shared by both internal/upstream and
// internal/downstream. Each converter translates in both directions between
// that protocol's wire format and the provider-neutral core/ir types:
//
...
//
// Adding a new protocol = one new file (e.g. openai.go) implementing
// UpstreamConverter/DownstreamConverter. No changes to server, pipeline,
// upstream, or downstream code required.
package wireformat
...
// maxSSELineSize bounds a single scanned SSE line for StreamToIR implementations.
// Raised from bufio.Scanner's 64KiB default, which silently truncates the stream
// on long tool-call argument or thinking-block deltas; capped to bound memory
// use against a malformed or hostile upstream.
const maxSSELineSize = 16 << 20 // 16 MiB
