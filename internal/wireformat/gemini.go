...
func (*GeminiConverter) ResponseToIR(body []byte) (*ir.IRResponse, error) {
	...
	for _, part := range candidate.Content.Parts {
		...
		if part.FunctionCall != nil {
			...
			hasToolUse = true
		} else if part.ThoughtSignature != "" && lastToolID != "" {
			// Gemini sometimes ships the signature on a trailing part with no
			// functionCall of its own (e.g. an empty-text part) rather than
			// co-located with the call — attach it to the most recently
			// emitted tool call in this response.
			geminiThoughtSigs.store(lastToolID, part.ThoughtSignature)
		}
	}
...
func (*GeminiConverter) StreamToIR(ctx context.Context, body io.Reader) <-chan ir.StreamEvent {
	...
	go func() {
			...
			for _, candidate := range chunk.Candidates {
				for _, part := range candidate.Content.Parts {
					...
					if part.FunctionCall != nil {
						...
						// Gemini can ship the signature on a later chunk's
						// trailing part with no functionCall of its own
						// (e.g. an empty-text part) instead of co-located
						// with the call — attach it to the most recently
						// emitted tool call in this stream.
						geminiThoughtSigs.store(lastToolID, part.ThoughtSignature)
					}
				}
		...
		if err := scanner.Err(); err != nil {
			slog.Error("gemini stream scanner error", "error", err)
		}
...
func convertIRParts(parts []ir.IRContentPart, allMsgs []ir.IRMessage) ([]types.GeminiPart, error) {
	...
	for _, p := range parts {
		...
		if p.ToolUse != nil {
			...
			sig := geminiThoughtSigs.lookup(p.ToolUse.ID)
			if sig == "" {
				// Cache miss (TTL expiry, a miroxy restart mid-conversation, or a
				// call that predates thinking mode) — Gemini's validator still
				// requires every functionCall in an ongoing turn to carry a
				// signature once one has been used, so fall back to the sentinel
				// it documents for exactly this case rather than send none and
				// get a hard 400. See https://ai.google.dev/gemini-api/docs/thought-signatures.
