...
func TestThinkingBlock_RoundTrip(t *testing.T) {
	req := &types.MessageRequest{
		Model:     "claude-haiku",
		MaxTokens: 100,
		Messages: []types.Message{{Role: "assistant", Content: mustJSON([]types.ContentBlock{
	...
	if len(irReq.Messages) != 1 || len(irReq.Messages[0].Parts) != 2 {
		t.Fatalf("want 1 message with 2 parts, got %+v", irReq.Messages)
	}
	reasoning := irReq.Messages[0].Parts[0].Reasoning
	if reasoning == nil {
		t.Fatalf("thinking block was dropped, want IRReasoningPart")
	}
	if reasoning.Text != "Let me think about this." || reasoning.Signature != "sig-abc" {
		t.Errorf("Reasoning: %+v", reasoning)
	}

	// Round-trip back out: the re-encoded wire block must match the original.
	wireMsg := messageFromIR(irReq.Messages[0])
	var blocks []types.ContentBlock
	if err := json.Unmarshal(wireMsg.Content, &blocks); err != nil {
		t.Fatalf("unmarshal re-encoded content: %v", err)
	}
	if len(blocks) != 2 || blocks[0].Type != "thinking" {
		t.Fatalf("want [thinking, text], got %+v", blocks)
	}
	if blocks[0].Thinking != "Let me think about this." || blocks[0].Signature != "sig-abc" {
		t.Errorf("re-encoded thinking block: %+v", blocks[0])
	}
}
...
func TestRedactedThinkingBlock_RoundTrip(t *testing.T) {
	req := &types.MessageRequest{
		...
		MaxTokens: 100,
		Messages: []types.Message{{Role: "assistant", Content: mustJSON([]types.ContentBlock{
			{Type: "redacted_thinking", Data: "opaque-envelope-data"},
		})}},
	}
	irReq, err := AnthropicConverter{}.RequestToIR(req)
	if err != nil {
	...
	reasoning := irReq.Messages[0].Parts[0].Reasoning
	if reasoning == nil {
		t.Fatalf("redacted_thinking block was dropped, want IRReasoningPart")
	}
	if reasoning.Text != "" || reasoning.Signature != "opaque-envelope-data" {
		t.Errorf("Reasoning: %+v", reasoning)
	}
	...
	if len(blocks) != 1 || blocks[0].Type != "redacted_thinking" || blocks[0].Data != "opaque-envelope-data" {
		t.Fatalf("re-encoded redacted_thinking block: %+v", blocks)
	}
}

func TestResponseToIR_ThinkingBlock(t *testing.T) {
	...
	irResp := AnthropicConverter{}.ResponseToIR(resp)
	if len(irResp.Content) != 2 || irResp.Content[0].Reasoning == nil {
		t.Fatalf("want reasoning block first, got %+v", irResp.Content)
	}
	if irResp.Content[0].Reasoning.Text != "Reasoning..." || irResp.Content[0].Reasoning.Signature != "sig-xyz" {
		t.Errorf("Reasoning: %+v", irResp.Content[0].Reasoning)
	}

	back := AnthropicConverter{}.ResponseFromIR(irResp, "msg_1", "claude-haiku")
	if len(back.Content) != 2 || back.Content[0].Type != "thinking" {
		t.Fatalf("re-encoded response: %+v", back.Content)
	}
	if back.Content[0].Thinking != "Reasoning..." || back.Content[0].Signature != "sig-xyz" {
		t.Errorf("re-encoded thinking block: %+v", back.Content[0])
	}
}
