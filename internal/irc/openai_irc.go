...
type oaiMessage struct {
	...
	Content    *string       `json:"content"`           // nil → JSON null (valid for assistant+tool_calls)
...
func buildOAIRequestBody(irReq *ir.IRRequest, model string, maxTemp float64) ([]byte, error) {
	...
	if irReq.System != "" {
		s := irReq.System
		msgs = append(msgs, oaiMessage{Role: "system", Content: &s})
	}
	...
	for _, msg := range msgs {
		...
		case "user":
			...
			if len(textParts) > 0 {
				...
				result = append(result, oaiMessage{Role: "user", Content: &s})
			}
...
func AnthropicToOAIResponseBody(resp *types.MessageResponse, model string) []byte {
	var textParts []string
...
// StreamAnthropicSSEToOAI reads Anthropic SSE events from in and writes OpenAI
// SSE chunks to w, flushing after each chunk.
//
// The function owns reading from `in` until it receives a message_stop event or
// ctx is cancelled. The caller is responsible for calling c.ReleaseUpstream after
// this function returns (to release the upstream connection and credential).
func StreamAnthropicSSEToOAI(
	ctx context.Context,
	in <-chan types.SSEEvent,
	w io.Writer,
	...
	writeChunk(oaiFrontendChunkChoice{
		Delta: oaiFrontendDelta{Role: role},
	}, nil)

	// State: track Anthropic content block index → tool call output index.
	toolCallByBlock := map[int]int{} // anthropic block index → our tool_calls[] index
	nextToolIdx := 0
	...
	for {
		...
		case ev, ok := <-in:
			...
			// Use JSON round-trip for safe type extraction from ev.Data (any).
			raw, _ := json.Marshal(ev.Data)

			switch ev.Event {
			case "content_block_start":
				var d types.ContentBlockStartData
				if json.Unmarshal(raw, &d) != nil {
					continue
				}
				if d.ContentBlock.Type == "tool_use" {
					toolCallByBlock[d.Index] = nextToolIdx
					// First tool call delta: announces id + name, empty arguments.
					writeChunk(oaiFrontendChunkChoice{
						Delta: oaiFrontendDelta{
							ToolCalls: []oaiFrontendTCDelta{{
								...
								Type:     "function",
								Function: oaiFuncCall{Name: d.ContentBlock.Name},
							}},
						},
					}, nil)
					nextToolIdx++
				}
				// text blocks: no chunk emitted on start (OpenAI has no content_block_start)
				...
				case "input_json_delta":
					...
					writeChunk(oaiFrontendChunkChoice{
						Delta: oaiFrontendDelta{
							ToolCalls: []oaiFrontendTCDelta{{
								Index:    tcIdx,
								Function: oaiFuncCall{Arguments: d.Delta.PartialJSON},
							}},
						},
					}, nil)
				}
			...
			case "message_delta":
				...
				reason := mapAnthropicStopReason(d.Delta.StopReason)
				// Finish chunk.
				writeChunk(oaiFrontendChunkChoice{
					Delta:        oaiFrontendDelta{},
					FinishReason: &reason,
				}, nil)
				// Usage chunk (empty choices, usage attached).
				if d.Usage.OutputTokens > 0 || d.Usage.InputTokens > 0 {
					usage := &oaiUsage{
